...
function Methodology() {
    ...
    return (<main_layout_1.MainLayout>
      ...
      <h2 className="text-2xl font-display font-bold mb-6 flex items-center gap-2">
        <lucide_react_1.Target className="text-primary w-6 h-6"/> Techniques d'étude prouvées
      </h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        {tips.map(function (tip, i) { return (<card_1.Card key={i} className="rounded-2xl border-border shadow-sm hover:shadow-md transition-shadow">
            <card_1.CardContent className="p-6 sm:p-8 flex gap-6">
              <div className={"w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ".concat(tip.bg, " ").concat(tip.color)}>
      ...
      <div className="bg-card border border-border rounded-3xl p-8 md:p-10 shadow-sm">
        ...
        ].map(function (text, i) { return (<div key={i} className="flex gap-4 items-start">
...
function Badge(_a) {
    ...
    return <span className={"inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 ".concat(className)}>{children}</span>;
}
