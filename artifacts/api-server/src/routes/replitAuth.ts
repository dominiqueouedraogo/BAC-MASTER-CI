...
async function upsertUser(claims: Record<string, unknown>) {
  ...
  const displayName =
    [firstName, lastName].filter(Boolean).join(" ") || email || "Student";

  const conditions = [eq(usersTable.replitId, replitId)];
  ...
  if (existing) {
    const [updated] = await db
      ...
      .set({
        replitId,
        avatarUrl: profileImageUrl ?? existing.avatarUrl,
        updatedAt: new Date(),
      })
...
function toSessionUser(user: typeof usersTable.$inferSelect) {
  return {
    id: String(user.id),
